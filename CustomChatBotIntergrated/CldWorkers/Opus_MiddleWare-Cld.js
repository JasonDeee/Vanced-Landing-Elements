// Cloudflare Worker: Opus Middleware Proxy
// Xử lý các request từ client (OpusBuilder.js) và tạo userID nếu cần
// Node.js local dev
const fs = require("fs");
...
try {
  // Cloudflare Worker hỗ trợ import static assets qua __STATIC_CONTENT hoặc Bundler
  // Ở môi trường node hoặc local, dùng fs.readFileSync
  if (typeof __STATIC_CONTENT !== "undefined") {
    // Với môi trường Workers Bundler (ví dụ Wrangler)
    Opus_Tunned_Data = await __STATIC_CONTENT.get("OpusTunned_Data.txt", {
      type: "text",
    });
...
export default {
  async fetch(request, env, ctx) {
    // Gọi loadOpusTunnedData() trong handler chính nếu cần
    // await loadOpusTunnedData();
    ...
    if (request.method === "POST") {
      ...
      if (url.pathname === "/api/opus") {
        ...
        if (body.requestType === "PageLoadRequestData") {
          ...
          if (body.cloudstorage && Array.isArray(body.cloudstorage)) {
            try {
              ...
              if (selectedPairs.length > 0) {
                // Gửi đúng định dạng [ [type, id], ... ]
                const configsArr = selectedPairs;
                opusDebug.push({ step: "configsArr", data: configsArr });
                mappedSlots = await fetchMultipleProductsFromConfigs(
                  configsArr
                );
          ...
          return new Response(
            JSON.stringify({ userID, mappedSlots, chatHistory, opusDebug }),
            {
              status: 200,
              headers: {
                "content-type": "application/json",
        ...
        // Các loại request khác sẽ xử lý sau
        return new Response(JSON.stringify({ error: "Unknown requestType" }), {
          status: 400,
          headers: {
            "content-type": "application/json",
            "Access-Control-Allow-Origin": "*",
